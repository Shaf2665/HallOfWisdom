 ...
 * this package spawns and terminates real Codex processes, and an
 * accidental real kill in a test could target an unrelated PID (including,
 * in the worst case, this very session). Tests always inject a fake.
 */
...
export const realPosixGroupKiller: PosixGroupKiller = {
  killGroup(pid, signal) {
    try {
      ...
      // grandchild processes Codex itself spawns (e.g. a sandboxed shell
      // command), not just the direct child.
...
export interface ProcessTreeOptions {
  ...
  /**
   * Reused as-is from the sanitized child environment already built for
   * the main `codex` process (see `environment.ts`'s
   * `buildChildEnvironment`) — it already carries `PATH`/`SYSTEMROOT`/
   * `WINDIR`/`COMSPEC`/`TEMP`/`TMP`, which is what `taskkill.exe` needs to
   * reliably resolve and run.
   */
 ...
 * the caller, `CodexRun`, before calling this) is the only "graceful"
 * step available there; this function is a deliberate no-op on Windows,
 * with `forceTerminateProcessTree`'s `taskkill /T /F` doing the actual,
 * guaranteed cleanup once the bounded grace period elapses.
 */
 ...
 * never a constructed command string. `/T` targets the whole tree (not
 * just the direct child); `/F` forces termination.
 */
