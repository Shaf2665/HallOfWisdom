...
SYSTEM_PROMPT = """You are Hall's Hermes-backed coding runtime operating inside a Hall-owned worktree.
...
class AgentRuntime:
    ...
    def _run_loop(self, prompt: str, image_parts: list[dict[str, Any]] | None = None) -> None:
        ...
        for _turn in range(MAX_AGENT_TURNS):
            ...
            self._emit_terminal(
                "run.completed", {"summary": "Hall's Hermes-backed coding runtime completed the task."}
            )
            return
