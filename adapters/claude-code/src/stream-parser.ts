import { classifyNativeLine, type ParsedNativeMessage } from "./native-messages.js";
...
/**
 * Incrementally parses Claude Code's newline-delimited stream-json stdout.
 * Handles both LF and CRLF line endings, tolerates lines split across
 * multiple `push()` calls (Node stream chunk boundaries never align with
 * message boundaries), and never lets one malformed or oversized line
 * abort parsing of the rest of the stream — each line's outcome is
 * reported independently; deciding whether a given outcome is fatal for
 * the run as a whole is `ClaudeCodeRun`'s job, not this parser's.
 *
...
export class StreamParser {
  ...
  push(chunk: string): readonly StreamLineOutcome[] {
    ...
    // has terminated it yet. Without this, a stream chunked without any
    // newline for a long time (or ever) could grow #buffer unbounded even
    // though maxLineLength is checked inside #processLine — that check
    // only ever runs on a line a "\n" has already completed.
