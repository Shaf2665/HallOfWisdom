...
/**
 * Runs one short-lived `git` invocation to completion (or until it's
 * killed for exceeding `timeoutMs`), capturing bounded stdout/stderr.
 * Every `git` command `GitWorktreeManager` issues (`rev-parse`, `status
 * --porcelain`, `worktree add/remove/prune`, `diff --numstat`, etc.) is
 * expected to complete quickly and produce a small, bounded amount of
 * output, and none of them need to write anything to stdin — stdin is
 * closed immediately so a command can never block waiting for input that
 * will never arrive.
 */
